...
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu ] && exit || \
	 ...
	 $(call fetch-git-tree,armnn,apps/eiq) && \
	 ...
	 if [ ! -f $(eIQDESTDIR)/usr/local/lib/libprotobuf.so ]; then \
	     flex-builder -c protobuf -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -f $(eIQDIR)/caffe/build/src/caffe/proto/caffe.pb.cc ]; then \
	    flex-builder -c caffe -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 ...
	 if [ ! -f $(eIQDIR)/flatbuffer/libflatbuffers.a ]; then \
	     flex-builder -c flatbuffer -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -f $(eIQDESTDIR)/usr/local/lib/libarm_compute.so ]; then \
	     flex-builder -c armcl -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -f $(eIQDIR)/onnx/onnx/onnx.pb.cc ]; then \
	     flex-builder -c onnx -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 \
	 cd $(eIQDIR)/armnn && \
	 git reset --hard && patch -p1 < $(FBDIR)/packages/apps/eiq/patch/0001-fix-pyarmnn-cross-compile-issue.patch && \
