...
.PHONY: secure_obj
secure_obj:
ifneq ($(CONFIG_APP_SECURE_OBJ)$(FORCE), "n")
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu -o $(DISTROSCALE) = tiny -o $(DISTROSCALE) = lite ] && exit || \
	 $(call fbprint_b,"secure_obj") && \
	 $(call fetch-git-tree,secure_obj,apps/security) && \
	 if [ ! -f $(DESTDIR)/usr/local/include/openssl/opensslconf.h ]; then \
	     flex-builder -c openssl -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
	 if [ $(CONFIG_APP_OPTEE) != y ]; then \
	     $(call fbprint_e,"Please enable CONFIG_APP_OPTEE to y in configs/$(CFGLISTYML)"); exit 1; \
	 fi && \
	 ...
	 if [ ! -f $(DESTDIR)/lib/libteec.so ]; then \
	     flex-builder -c optee_client -r $(DISTROTYPE):$(DISTROSCALE) -f $(CFGLISTYML); \
	 fi && \
