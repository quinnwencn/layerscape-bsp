...
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu -o $(DISTROSCALE) = lite -o $(DISTROSCALE) = tiny ] && exit || \
	 ...
	 if [ ! -d $(SECDIR)/secure_obj/securekey_lib/include ]; then \
	     flex-builder -c secure_obj -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
	 cd $(SECDIR)/libpkcs11 && \
