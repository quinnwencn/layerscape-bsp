...
ifeq ($(DESTARCH),arm64)
	@if [ $(DISTROTYPE) != ubuntu -a $(DISTROTYPE) != yocto -o $(DISTROSCALE) = desktop -o \
	      $(DISTROSCALE) = lite -o $(DISTROSCALE) = tiny ]; then \
	     echo option DISTROSCALE $(DISTROSCALE) is not enabled for DPDK, please use main or devel; exit; \
	 fi && \
	 $(call fbprint_b,"dpdk") && \
	 $(call fetch-git-tree,dpdk,apps/networking) && \
	 ...
	 if [ ! -f $(DESTDIR)/usr/local/lib/libcrypto.so ]; then \
	     flex-builder -c openssl -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
