...
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu -o $(DISTROSCALE) = desktop -o \
	   $(DISTROSCALE) = lite -o $(DISTROSCALE) = tiny ] && exit || \
	 $(call fbprint_b,"pktgen_dpdk") && \
	 $(call fetch-git-tree,pktgen_dpdk,apps/networking) && \
	 if [ ! -d $(RFSDIR)/lib/aarch64-linux-gnu ]; then \
	 ...
	 sed -e 's%@TARGET_CROSS@%$(CROSS_COMPILE)%g' -e 's%@TARGET_ARCH@%aarch64%g' \
             ...
             $(FBDIR)/packages/rfs/misc/meson/cross-compilation.conf > $(NETDIR)/pktgen_dpdk/cross-compilation.conf && \
