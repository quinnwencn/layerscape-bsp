...
.PHONY: xserver
xserver:
ifeq ($(CONFIG_APP_XSERVER), "y")
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu -o $(DISTROSCALE) != desktop ] && exit || \
	 $(call fetch-git-tree,xserver,apps/graphics) && \
	 ...
	 if [ ! -f $(DESTDIR)/usr/lib/libGLES_CL.so ]; then \
	     flex-builder -c gpulib -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -f $(DESTDIR)/usr/lib/libdrm.so ]; then \
	     flex-builder -c libdrm -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -f $(DESTDIR)/usr/include/wayland-client.h ]; then \
	     flex-builder -c wayland -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
	 if [ ! -d $(DESTDIR)/usr/share/wayland-protocols ]; then \
	     flex-builder -c wayland_protocols -r $(DISTROTYPE):$(DISTROSCALE) -a $(DESTARCH) -f $(CFGLISTYML); \
	 fi && \
	 cd $(GRAPHICSDIR)/xserver && \
	 if [ ! -f .patchdone ]; then \
	     git am $(FBDIR)/packages/apps/graphics/patch/xserver/*.patch && touch .patchdone; \
	 fi && \
	 if [ ! -d $(RFSDIR)/usr/share/wayland-protocols ]; then \
	     sudo cp -rf $(DESTDIR)/usr $(RFSDIR)/; \
	 fi && \
	 cp -f $(FBDIR)/packages/apps/graphics/dri.pc $(DESTDIR)/usr/lib/pkgconfig/ && \
	 ...
	 install -m 0755 $(FBDIR)/packages/apps/graphics/gpuconfig $(DESTDIR)/etc && \
	 install -m 0644 $(FBDIR)/packages/apps/graphics/gpuconfig.service $(DESTDIR)/usr/lib/systemd/system && \
