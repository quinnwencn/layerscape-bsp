...
ifeq ($(DESTARCH),arm64)
	@[ $(DISTROTYPE) != ubuntu -o $(DISTROSCALE) != desktop ] && exit || \
	 $(call fbprint_b,"libdrm") && $(call fetch-git-tree,libdrm,apps/graphics) && cd $(GRAPHICSDIR)/libdrm && \
	 ...
	 sed -e 's%@TARGET_CROSS@%$(CROSS_COMPILE)%g' -e 's%@TARGET_ARCH@%aarch64%g' \
	     ...
	     $(FBDIR)/packages/rfs/misc/meson/cross-compilation.conf > build/cross-compilation.conf && \
